    ...
    def accounts_page() -> None:
        ...
        with page_container("/accounts"):
            ...
            with ui.card().classes("bp-card w-full p-6"):
                ...
                with ui.grid(columns="repeat(3, minmax(180px, 1fr))").classes("w-full gap-6"):
                    with ui.column().classes("gap-1"):
                        ui.label("Anzahl Konten").classes("bp-muted")
                        ui.label(str(len(accounts))).classes("text-2xl font-bold")
                    with ui.column().classes("gap-1"):
                        ui.label("Gesamter Startsaldo").classes("bp-muted")
                        ui.label(money(total_start)).classes("text-2xl font-bold bp-money")
                    with ui.column().classes("gap-1"):
                        ui.label("Aktueller Gesamtsaldo").classes("bp-muted")
                        ui.label(money(total_balance)).classes(f"text-2xl font-bold bp-money {'bp-positive' if total_balance >= 0 else 'bp-negative'}")
